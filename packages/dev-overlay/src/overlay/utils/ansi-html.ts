...
function getExtendedColor([mode, ...seq]: [number, ...number[]]): string | null {
    switch (mode) {
        case 2: {
...
function updateState(seq: string, { attr, fg, bg }: AnsiState): AnsiState | null {
    // check for extended color sequences 38;... / 48;....
    if (/^[34]8;/.test(seq)) {
        const color = getExtendedColor(seq.split(';').slice(1).map(Number) as [number, ...number[]]);
        return seq.startsWith('3') ? { attr, fg: color, bg } : { attr, fg, bg: color };
    }
    // ignore other complex sequences
    if (seq.includes(';')) return null;

    const code = Number(seq);
    // reset code
    if (code === 0) {
        return { attr: 0, fg: null, bg: null };
    }
    // set attr code
    if ([1, 2, 3, 4, 7, 9].includes(code)) {
        const i = [1, 2, 3, 4, 9, 7].indexOf(code);
        return { attr: attr | (1 << i), fg, bg };
    }
    // reset attr code
    if ([22, 23, 24, 27, 29].includes(code)) {
        ...
        return { attr: attr & mask, fg, bg };
    }
    // foreground color
    if (code >= 30 && code <= 37) {
        return { attr, fg: ANSI_COLORS[code % 10]!, bg };
    }
    // background color
    if (code >= 40 && code <= 47) {
        return { attr, fg, bg: ANSI_COLORS[code % 10]! };
    }
    // foreground bright color
    if (code >= 90 && code <= 97) {
        return { attr, fg: ANSI_COLORS[8 + (code % 10)]!, bg };
    }
    // background bright color
    if (code >= 100 && code <= 107) {
        return { attr, fg, bg: ANSI_COLORS[8 + (code % 10)]! };
    }
    // foreground reset
    if (code === 39) return { attr, fg: null, bg };
    // background reset
    if (code === 49) return { attr, fg, bg: null };
 ...
 */
function resolveStyle(attr: number, fg: string | null, bg: string | null): string {
